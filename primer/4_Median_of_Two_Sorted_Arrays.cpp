...
private:
    int getKthElement(std::vector<int>& nums1, std::vector<int>& nums2, int k) {
        ...
        while (true) {
            ...
            if (pivot1 <= pivot2) {
                k -= newIndex1 - index1 + 1;    // 去除所有nums1中[0, pivot1]的元素
                index1 = newIndex1 + 1;         // 下一轮搜索起点index1变为pivot1右边的第一个元素
            } else {
                k -= newIndex2 - index2 + 1;    // 去除nums2中[0, pivot2]的元素
                index2 = newIndex2 + 1;         // 下一轮搜索起点index2变为pivot2右边的第一个元素
            }
        }
    }
};
